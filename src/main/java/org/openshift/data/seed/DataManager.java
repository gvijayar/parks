...
public class DataManager {
	...
	public String getParkCoordinates(String lon1, String lat1, String lon2,
			String lat2) {

		String query = "SELECT gid, " + "name, " + "ST_X(the_geom) as lon,"
				+ "ST_Y(the_geom) as lat " + "FROM parkdata t "
				+ "WHERE ST_Intersects( ST_MakeEnvelope(" + lon1 + ", " + lat1
				+ ", " + lon2 + ", " + lat2 + ", 4326), " + "t.the_geom)";
		return query;
	}
	...
	public String initializeDatabase() {
		return "CREATE TABLE parkdata ( gid serial NOT NULL, "
				...
				+ "CHECK (st_srid(the_geom) = 4326) ) " + "WITH ( OIDS=FALSE )";
	}
